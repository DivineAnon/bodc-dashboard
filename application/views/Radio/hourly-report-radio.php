    ...
    <div class="page-content container-fluid">
      <div class="row" data-plugin="matchHeight" data-by-row="true">
        <div class="col-xl-12 col-md-12 text-left mb-15">
          <h2 style="color: #000000; margin-left: 65px;"><b>HOURLY REPORT (RADIO)</b></h2>
        </div>
      </div>
      <div class="container">
        <?php echo form_open_multipart(base_url().'Radio/add_data_radio', 'class="form-inline mb-20"'); ?>
            <p style="color: #000000; margin-left: 50px; margin-top: 10px !important; margin-right: 5px;">nomor unit :</p>
            <select class="form-control select2" name="no_unit" id="no_unit" style="margin-right: 15px !important; width: 14%;">
                <option disabled selected>-- Nomor Unit --</option>
                <?php
                    if(!empty($unit)){
                        for($i=0;$i < count($unit['unit']);$i++){
                            $no_unit_admin = $unit['unit'][$i];
                        ?>
                        <option value="<?php echo $no_unit_admin;?>"><?php echo $no_unit_admin;?></option>
                        <?php
                        }
                    }
...
<!--            <input type="text" class="form-control" name="no_unit" placeholder="nomor unit"-->
<!--            style="margin-right: 5px;" autocomplete="off" required>-->
            <!-- <input type="date" class="form-control" id="tgl_aktifitas" name="tgl_aktifitas" placeholder="tanggal aktifitas" style="margin-right: 5px;" value="<?= date('Y-m-d'); ?>" required> -->
      ...
      <div class="page-content">
        ...
        <div class="panel" style="margin-left: 30px;">
          <header class="panel-heading text-center">
            ...
            <h1 class="panel-title" style="background:#0267ca; color: #fff; font-size: large; text-align: left;">Last
              Update Radio Data</h1>
          </header>
          <div class="panel-body mt-10">
            <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
              ...
              <tbody>
                <?php
                  if(!empty($hourly['date'])){
                    for ($i=0; $i < count($hourly['date']); $i++) {
                      $id = $hourly['id'][$i];
                      $created_date = $hourly['created_date'][$i];
                      $date = $hourly['date'][$i];
                      $no_unit = $hourly['no_unit'][$i];
                      $dt = $hourly['dt'][$i];
                      $operator = $hourly['operator'][$i];
                      $status = $hourly['status'][$i];
                ?>
                <tr>
                  ...
                  <td>
                    <a href="<?php echo base_url().'Radio/update_data_radio/'.$no_unit.'/'.$id;?>" class="button mr-10" style="font-size:large; text-decoration:none !important;">
...
<script>
  $(document).ready(function() {
      ...
      if (flashData) {
          ...
          } else if (flashData == 'failed no unit') {
              title = 'Oops! Something went wrong';
          ...
          if (title == null && msg == null) {
              ...
              swal({
              icon: 'error',
              title: 'No unit is already in database',
              showConfirmButton: false,
              timer: 2000
            })
          }
